...
def validate(fabric: L.Fabric, model: Model, val_dataloader: DataLoader, epoch: int = 0):
    ...
    if fabric.global_rank == 0:
        torch.save(state_dict, os.path.join(cfg.out_dir, f"20000_epoch-{epoch:03d}-f1_{f1_scores.avg:.2f}-ckpt.pth"))
        print("Epoch: ", epoch, "Path: ", os.path.join(cfg.out_dir, f"20000_epoch-{epoch:03d}-f1_{f1_scores.avg:.2f}-ckpt.pth"))
    model.train()
...
def main(cfg: Box) -> None:
    ...
    train_data, val_data = load_datasets(cfg, 1024) # model.model.image_encoder.img_size # 1024
